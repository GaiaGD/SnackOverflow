...
- **Spam prevention & CAPTCHA**: the lead form has no bot protection. The recommended approach is **Google reCAPTCHA v3** (invisible, score-based — no checkbox friction for the user) or **Cloudflare Turnstile** (privacy-friendly alternative). The token would be generated client-side on submit and verified server-side before the payload is forwarded to the CRM. For lighter-weight protection, a honeypot field (hidden from users, filled only by bots) can be added as a first layer with zero UX cost.
...
- **E2E tests**: Playwright smoke tests covering landing page render, CTA → external link navigation, and form submit flow.
...
- **CSS modules + design tokens over Tailwind**: Tailwind is fast to scaffold but creates friction with designers and makes systematic visual changes harder. The preferred direction is CSS modules per component paired with a design token layer — CSS custom properties defined once (`--color-bg`, `--color-accent`, `--color-highlight`) and consumed everywhere. This makes handoff cleaner and a full rebrand a one-file change rather than a grep-and-replace across every component.
...
- **Fluid typography with `clamp()`**: replace fixed `text-*` sizes with CSS `clamp()` so headings and body text scale smoothly between viewport widths without abrupt breakpoint jumps — e.g. `font-size: clamp(1.5rem, 4vw, 3.75rem)` on the hero headline. This is particularly valuable for the Climate Crisis display font where size has a strong visual impact.
- **CTA as a standalone CMS content type**: `CtaButton` is already a reusable React component (renders as `<a>` or `<button>` depending on whether a URL is provided), currently driven by `ctaLabel` and `ctaUrl` fields on the hero block. Depending on team needs, it could be promoted to its own Contentful content type — allowing editors to create a CTA once and reference it across multiple blocks without duplicating data.
